...
class SQLiteWorkflowJobStore:
    ...
    def _initialize(self) -> None:
        with self._connect() as connection:
            connection.execute("""CREATE TABLE IF NOT EXISTS maxxvoice_workflow_jobs (
                id TEXT PRIMARY KEY, workflow TEXT NOT NULL, status TEXT NOT NULL,
                created_at TEXT NOT NULL, started_at TEXT, completed_at TEXT,
                result_json TEXT, error TEXT, payload_json TEXT, idempotency_key TEXT,
                UNIQUE(workflow, idempotency_key))""")
            columns = {row["name"] for row in connection.execute("PRAGMA table_info(maxxvoice_workflow_jobs)").fetchall()}
    ...
    def create(self, workflow: str, job_id: Optional[str] = None,
        ...
        job, _ = self.create_or_get(workflow, job_id=job_id, payload=payload, idempotency_key=idempotency_key)
        return job
        ...
        try:
            ...
            if idempotency_key:
                row = connection.execute(
                    ...
                    (workflow, idempotency_key)).fetchone()
                if row is not None:
                    connection.commit()
            ...
            connection.execute(
                """INSERT INTO maxxvoice_workflow_jobs
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (job.id, job.workflow, job.status.value, job.created_at, job.started_at,
                 job.completed_at, json.dumps(job.result, ensure_ascii=False) if job.result is not None else None,
                 job.error, json.dumps(job.payload, ensure_ascii=False), job.idempotency_key))
            connection.commit()
            return job, True
        except Exception:
            connection.rollback()
    ...
    def get(self, job_id: str) -> Optional[WorkflowJob]:
        with self._connect() as connection:
            row = connection.execute("SELECT * FROM maxxvoice_workflow_jobs WHERE id = ?", (job_id,)).fetchone()
        return self._from_row(row) if row is not None else None

    def list(self, workflow: Optional[str] = None) -> list[WorkflowJob]:
        with self._connect() as connection:
            if workflow:
                rows = connection.execute(
                    "SELECT * FROM maxxvoice_workflow_jobs WHERE workflow = ? ORDER BY created_at ASC", (workflow,)
                ).fetchall()
            else:
                rows = connection.execute("SELECT * FROM maxxvoice_workflow_jobs ORDER BY created_at ASC").fetchall()
    ...
    def update(self, job: WorkflowJob) -> WorkflowJob:
        with self._connect() as connection:
            cursor = connection.execute(
                """UPDATE maxxvoice_workflow_jobs SET workflow = ?, status = ?, created_at = ?,
                   started_at = ?, completed_at = ?, result_json = ?, error = ?,
                   payload_json = ?, idempotency_key = ? WHERE id = ?""",
                (job.workflow, job.status.value, job.created_at, job.started_at, job.completed_at,
                 json.dumps(job.result, ensure_ascii=False) if job.result is not None else None,
                 job.error, json.dumps(job.payload, ensure_ascii=False), job.idempotency_key, job.id))
            connection.commit()
    ...
    def _from_row(row: sqlite3.Row) -> WorkflowJob:
        return WorkflowJob(
            ...
            error=row["error"], payload=json.loads(row["payload_json"]) if row["payload_json"] else {},
